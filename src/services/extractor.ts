...
function buildSystem(style: PitchStyle): string {
  ...
  '- A TOPICAL site often prices its OWN subject apart from everything else ("Tennis-related',
  '  guest post €350, Non-tennis guest post €500"). The OFF-TOPIC / "non-X" figure is the',
  '  STANDARD rate → category "regular" (€500 here): a post about anything else is what a',
  '  general advertiser buys. The site\'s own subject becomes a niche of its own with its own',
  '  key (tennis → "tennis", €350). Emit BOTH, and never let the cheaper on-topic figure',
  '  become the regular rate just because it is listed first.',
  '',
  ...(style === 'casino' ? LITERAL_CASINO : LITERAL_BROAD),
  '',
  'For each offer:',
  '- canPost: "yes" they will publish a guest post in this niche, "no" they EXPLICITLY refuse',
  '  it, "maybe" they addressed the niche but were vague or conditional about it.',
  ...
  '',
  'A REFUSAL IS AN OFFER. A niche the owner explicitly says they will NOT publish must be',
  'emitted, never dropped — it is the only thing that can rule this site out for that niche:',
  '- They name a niche they refuse → ONE offer for it, canPost "no", priceRaw "" (a refusal',
  '  carries no price, and that is correct — do not omit the offer for lack of a figure).',
  '  "Guest post $99, casino $500, but we don\'t do VPN" → THREE offers: regular yes "$99",',
  '  casino yes "$500", vpn NO with priceRaw "".',
  '- A BLANKET refusal of the whole grey/sensitive area with no vertical named ("no grey',
  '  niches", "we do not accept sensitive topics", "nothing gambling-related or adult") →',
  '  ONE offer, category "sensitive", canPost "no", priceRaw "". Do NOT expand it into',
  '  separate casino/vpn/cbd/... cells — the umbrella covers them downstream.',
  '- Refusals are LITERAL, exactly like prices: a "no" covers ONLY the niche(s) they actually',
  '  named. Refusing casino says NOTHING about vpn or cbd — emit no cell for those at all.',
  '',
  'SILENCE IS NOT A REFUSAL — and it is not an acceptance either. A niche the reply never',
  'mentions gets NO offer: not a "no", not a "maybe", not a blank price. An ABSENT cell is',
  'exactly how we record "they never said", and it is read downstream as "possibly available,',
  'ask them" — whereas a wrongly emitted "no" rules the site out for that niche for good.',
  'So emit a cell ONLY for a niche the owner priced, accepted, refused, or hedged on IN THIS',
  'REPLY, and never round a partial answer up into a full niche list.',
  '',
  'Other rules:',
  ...
  '  "decline" = a BLANKET refusal covering the whole site — not interested at all, they do',
  '  not sell posts, do not contact us. "other" = none. If they gave ANY price/willingness, it',
  '  is "answer", never "holding".',
  '  Refusing SOME niches while pricing or accepting others is NOT a "decline" — it is an',
  '  "answer" that happens to carry canPost "no" cells ("guest post $99, but no casino" →',
  '  intent "answer"). A "decline" drops the entire site from our list, so reserve it for a',
  '  total refusal and never infer it from a niche-level no.',
